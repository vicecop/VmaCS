...
{
    private static Vk _vkApi => InstanceCreationExample.VkApi;
    ...
    {
        var res = _vkApi.GetFenceStatus(_device, fence);
    ...
    private unsafe void ScheduleThreadMethod()
    {
        ...
        while (_state == 0)
        {
            while (_fenceQueue.TryDequeue(out var tuple))
            {
                ...
                res = _vkApi.GetFenceStatus(_device, fence);
            ...
            res = _vkApi.WaitForFences(_device, (uint)list.Count, list.BasePointer, false, 5 * 1_000_000);
    ...
    private struct UnmanagedList<T> where T : unmanaged
    {
        ...
        public readonly unsafe T* BasePointer => (T*)Unsafe.AsPointer(ref MemoryMarshal.GetArrayDataReference(_array));
